...
pub trait OwnedValue: private::Sealed {
    fn handle<'a>(&'a self) -> LuaInnerHandle<'a>;

    fn try_detach(&self) -> Result<(), Error> {
        let mut guard = self.handle().0.try_borrow_mut()?;
    ...
    fn try_anchor_to(&self, other: impl OwnedValue) -> Result<(), Error> {
        let other_guard = other.handle().0.try_borrow()?;
        {
            let self_guard = self.handle().0.try_borrow()?;
        ...
        let mut self_guard = self.handle().0.try_borrow_mut()?;
